Ext.define('PDA.view.main.settings.DRPServerSettings', {
    ...
    initComponent: function () {
        ...
        me.listeners = {
            ...
            activate: function (cmp) {
                console.log('===activate drpserver===');
                me.loadData();
            }
        };
    ...
    setData: function (test) {
        ...
        if (me.dataValidation()) {
            ...
            var api_url = 'pda_api/' + window.sessionStorage.getItem('host') + '/settings/drprophet';
            if (test) {
                api_url += ('?action=test');
            }
            me.getController().onSaveClick({url: api_url, metadata: metadata, title: 'DRProphet', test: test});
        }
    },
    ...
    loadData: function () {
        ...
        PDA.view.main.Utils_Api.callApi({
            ...
            timeout: 120000,
            title: 'DRProphet',
            apiUrl: 'pda_api/' + window.sessionStorage.getItem('host') + '/settings/drprophet',
            apiCallback: function (options, success, response) {
                console.log('===drprophet: ', response);
                me.reiniPage();
                var j_data = Ext.JSON.decode(response.responseText, true) || null;
                if (success && j_data && j_data.res && j_data.success) {
                    if (j_data.res.drprophet && j_data.res.drprophet["host"] != undefined) {
                        me.down('#host').setValue(j_data.res.drprophet["host"]);
                    }
                    if (j_data.res.drprophet && j_data.res.drprophet["username"] != undefined) {
                        me.down('#username').setValue(j_data.res.drprophet["username"]);
                    }
                    if (j_data.res.drprophet && j_data.res.drprophet["password"] != undefined) {
                        me.down('#password').setValue(j_data.res.drprophet["password"]);
                    }
                }
            }
        });
